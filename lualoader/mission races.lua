...
while true do wait()
 ...
 if Star_mission_marker(5, 665.0, -621.2, 10.5) -- создать маркер миссии на карте. 
 then showtext(" race", 500,1)-- вывод названия миссии. 
   mycar = Createcar("MODEL_SANDKING", 670.0, -621.2, 10.5)
  set_radio(9,1)-- выключить радио.
  car = Createcar("MODEL_SANDKING", 665.0, -621.2, 10.5)  
  ped = set_ped_in_car(car, "BMYCR" ) -- создать педа в авто
  setcarangle(car, 180) -- уст угол авто.
  ma=create_marker(car) -- создать маркер над авто.
  setcarspeed(car, 35) setdrivingstyle(car, 2)   
  m = createmarker(1,3,392.1, -1497.3, 11.4) setsizemarker(m,3)  
  setcarangle(mycar, 180)
  putincar(player, mycar) -- переместить игрока в авто.
  fade(1,2900) --просветления, 600 время.  
  t = races()-- вернуть таймер
  go_to_route(car, road)-- авто следует маршуту.
  draw_corona(true, 4.5, 6, 0, 255, 0, 0,  392.1, -1497.3, 11.4)-- вкл корону

while true == Getflagmission() do  wait(100) -- цикл пока статус миссии не изменится. 
  if  car_in_radius(mycar, 392.1, -1497.3, 11.4, 6.0,6.0,2.0)
  then printmessage("~r~you win",2000,1)  miss(100)
  break
 end    
   if  car_in_radius(car, 392.1, -1497.3, 11.4, 6.0,6.0,2.0)-- авто в точке с радиусом.
