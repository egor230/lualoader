...
while lualoader == nil do wait() player = findplayer()-- получить игрока
 if Star_mission_marker(6, -264.6, -1214.6, 8.1) -- создать маркер миссии на карте. 
 then showtext("delivery of a case with a message", 1200,1)-- вывод названия миссии. 
 setcord(player, -255.3, -1215.5, 8.1) setangle(player, 242.0) -- установить угол для педа.
 car1 = Createcar(MODEL_DELUXO, -244.0, -1221.4, 8.1) setangle(car1, 184.0) -- установить угол для авто.
 fade(1,3900) --просветления, 600 время.
 wait(1200) x = 478.7  y = -1720.9 z = 15.0  
 sp = create_sphere( x, y, z, 1.5)-- создать сферу.   
 printmessage("~r~take the case from the lighthouse",3000,1) 
