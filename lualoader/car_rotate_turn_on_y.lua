...
while true do wait()-- задержка.
 ...
 if Keypress("VK_J") -- если клавиша H.
   then printmessage("end", 3000, 1)
 end  -- 
 if Keypress("VK_G") -- если клавиша H.
  then   --for s=0.001,360,0.005 do	 
  car_rotate_turn_on_y(car, 0.2,0.03)
...
--  end
   ...
   -- car_turn_on_z_with_delay(car,30, 30 )-- повернуть авто по оси y.
 -- create_marker_car(car) 
	-- wait(1100)
	 --car_turn_on_z_with_speed(car,35, 6 )
   -- for s=0.001,0.1,0.001 do	
	  -- car_rotate_turn_on_z(car, s,s)
	 -- wait(900)
  -- t = getcar_turn_on_z(car) 
   
   -- cord =  tostring(string.format("speed =%.3f, turn = %.3f, angle = %.3f  \n ",s,s, getcarangle(car)))
   -- ]]", resi = " .. tostring(getcar_resistance(car)).."\n"
  -- file = io.open('turn.txt', 'a') -- открыть файл для записи 
  -- file:write(cord)   file:close() 
     -- printmessage(tostring(string.format(" c =%.f. ", c)), 750,1 )
   -- wait(1350)
 -- end
  -- printmessage("end", 3000, 1)
 -- if Keypress("VK_S") -- если клавиша H.
  -- then show_save_menu()
  -- end
	 -- s = get_at_on_z(car,90)
	   -- set_at_on_y(car,s)
 
	  -- set_at_on_y(car,s)
-- if is_button_pressed(17)
	 -- car_setrotate_on_z(car,a)--setcord(player, 529.6, 370.1, 10.9)

	 -- s = get_at_on_z(car,90)
--	 wait(300)
   -- res = getcar_resistance(car)
 -- res1 = read_memory(0x68F590 , 4)-- постоянная для трения, равна 1.
 -- cord = "speed = "..tostring(s).." turn = "..tostring(t)..", angle = " .. tostring( getcarangle(car))..--[["\n"--]]", resi = " .. tostring(getcar_resistance(car)).."\n"
  -- file = io.open('turn.txt', 'a') -- открыть файл для записи 
  -- file:write(cord)    
  -- file:close()
  -- delete_entity(car)
    -- setcar_resistance(car, 10000) -- Уст силу сопротивления авто трению.
 -- if Keypress("VK_J") -- если клавиша H.
  -- then --carturnforce(car, 10,0,0, 0, 10,0)
  -- givemoney(10)
  -- for i=0.01,100, 0.01 do
  -- wait(300)
  -- carmovespeed_on_y(car, 0.1)
 --
-- create_marker_car(car) 
	   -- car_setrotate_on_z(car, z)
	   -- wait(300)
	   -- x,y,z = getcarcoordes(car)
	   -- setpedcoordes(player, x,y,z)
	    -- printmessage("angle = "..tostring(string.format("%.f %.f %.f", getcar_turn_on_z(car),getcar_turn_on_z(car),getcar_turn_on_z(car))), 1000,1 )
        -- d = getcarangle(car)
	   -- printmessage("angle = "..tostring(string.format("%.d", d)), 4500,1 )
 -- end 
 -- end
 
	 -- m = create_marker_car(car)
	  -- x,y,z = getcoordinates_on_y(player, 15)

   -- wait(3030) setcarrotate(20,10,0)
 -- car_moveforce_on_y(car, y)
-- car_turnforce_on_z(car, 0.3)
	-- create_sphere(x,y,z,2.0)
	--setcarangle(car, getpedangle(player))
   -- x,y,z = getcoordinates_on_y(player, 7)
	-- car1 = Createcar("MODEL_INFERNUS",x,y,z)
  -- wait(600)
	-- carturnforce(car, 10,0.2,0, 0, 30,0)
	 --car_turn_speed(car)
	 -- car_turnforce_on_y(car, 0.3)
  -- x,y,z = getcoordinates_on_y(player, 10)
	-- create_sphere(x,y,z,2.0)
	-- carturnfrictionforce(car, 0,0,0, 0, 70,0)
  -- carmoveforce(car, x, y,z)
 --[[
 Здравствуйте, всем.
 Есть вопрос.
 метод
 car->ApplyTurnForce(5,0,0, 0,25,0); поворачивает угол авто, который  был 0, стал 315, изменился на -45 градусов.
 
 необходимо чтобы поворачивай авто на нужный угол. 
 он задает силу вращения авто, но работает как-то странно, мне непонятна его логика
 есть ли какая-то формула соотношение между силой вращения и углом, допустим нужно повернуть авто на 45, задаем эти параметры. 
 вот что мне нужно.
 с этими параметрами  car->ApplyTurnForce(5,0,0, 0,25,0);  угол авто изменился на -45 градусов.
 
есть ли формула, чтобы с помощью этого метода поворачивать авто на конкретный заданный угол?
 что за взаимосвязь между углом поворота авто и силой вращения авто?
 
 вот какие углы получаются разными параметрами. 
 
 car->ApplyTurnForce(5,0,0, 0,25,0);  угол авто изменился на 45 градусов.
  carturnforce(car, 1,0,0, 0, 10,0) получаем угол 4
   carturnforce(car, 1,0,0, 0, 20,0) получаем угол    7
   carturnforce(car, 1,0,0, 0, 30,0)  получаем угол   10 						

	 carturnforce(car, 10,0,0, 0, 10,0) получаем угол 35
     carturnforce(car, 10,0,0, 0, 20,0) получаем угол  80
     carturnforce(car, 10,0,0, 0, 30,0)	получаем угол 135							   
 ]]
-- если 5 , 25, угол -45
   -- while  not is_car_damage_by_car(car, car1) 
   -- do wait()  
        -- d = car_lastweapondamage(car)
	   -- printmessage("money = "..tostring(string.format("%.d", d)), 500,1 )
   -- end 
   -- givemoney(3000000)
   -- printmessage("money = ", 3400,1 )
--  x,y,z = getcoordinates_on_y(player, 15)
