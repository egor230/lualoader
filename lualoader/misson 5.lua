...
while lualoader == nil do wait()
 ...
 if Star_mission_marker(8, 525.7, -77.5, 10.6) -- создать маркер миссии на карте. 
 then  showtext("Kill diasa", 500,1)-- вывод названия миссии.
 ...
 while true == getflagmission() do  wait() -- цикл пока статус миссии не изменттся.
  if not player_defined(ped) -- если враг убит.
   then  miss(3000)-- миссия выполнена сколько денег получено за выполнение.
   break   end  end-- 
  destroy()-- удалить все объекты, которые были созданны скриптом.	 
  end	 
  end
end 
