...
while lualoader == nil do
 ...
 if star_mission(player, "misson4") -- чит-код 
then set_traffic(0) mycar = Createcar(MODEL_STINGER,485.5, -111.2, 10.1) 
 showtext("driver", 500,1)-- вывод названия миссии.
...
ped = create_spec_ped("IGHLARY", 0.0,0.0,0.0) --
 ...
 restore_camera()
 t = races() 
   go_to_route(car, road)	printmessage("race",2000,1)
    while true == getflagmission() do  wait()
    b1 = follow_route_for_corona_for_playercar(mycar, road) 
   if b1 == true
	   then printmessage("you win",2000,1)  miss(100)
       break
       end 
   if finish_road(car, road)
	   then printmessage("you lose",2000,1)  
