...
while lualoader == nil do wait() player = findplayer()-- получить игрока
 ...
 fade(1,1900) --просветления, 600 время.   
...
while true == Getflagmission() do  wait() -- цикл пока статус миссии не изменится.--  
  if ped_in_point_in_radius(player,95.4, -1136.2, 10.5, 2.0,2.0,2.0) and not is_ped_in_car(player) and step == nil
   ...
   camera_at_point(92.39, -1131.74, 16.724, 2) --переместить камеру в координатах.
   wait(2000) ped_frozen(1) restore_camera()-- восстановить камеру.
   ...
   wait(1500) setcord(player, -241.7, -1362.3, 8.1 )
	setangle(player, 269.0 ) -- установить угол для педа.
	ped1 = create_spec_ped("IGMERC", -241.7, -1360.3, 8.1 )-- создать педа на координатах.
	wait(1500) fade(1,1100) --просветления, 600 время.    
	printmessage("~r~Take the Colonel's daughter to the Pole Position club", 3000,1)  
  ...
  if step == 2 
   then follow_the_leader(ped1)
    if ped_in_radius(player, 100.7, -1477.6, 10.1, 2.0,2.0,2.0) and ped_in_radius(ped1, 100.7, -1477.6, 10.1, 2.0,2.0,2.0) 
    ...
    then exitcar(ped1)-- пед выходит из авто.
       Remove(ped1) wait(1000) miss(1000)-- миссия выполнена сколько денег получено за выполнение.
   end
 step = nil
 break
 end
 if not player_defined(ped1)-- если мердес мертва, миссия провалена. 
 then printmessage("mesgess dead", 3000,1) end_mission("mission failed!")
 step = nil-- шаги обнуляются.
 break
 end 
 if not player_defined(p)-- если враг мертв.
 then Remove(p) removemarker(m)
 end
 end
 end
 end   
