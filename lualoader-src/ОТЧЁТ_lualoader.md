...
расширение API до 710 функций, см. раздел 3)
...
## 3. Lua-API (1506 зарегистрированных функций)

Из них **366 — оригинальные функции автора** (август–сентябрь 2026) и
**+1140 добавлены массовым переводом игрового API plugin-sdk**
...
2. Большое покрытие API (710 функций) — почти весь игровой функционал SCM и
...
## 6. Найденные проблемы и риски

**Баги (UB):**
1. `DoorsExample::EnableDoorEvent` (plugin.cpp ~строка 118):
   ```cpp
   CDamageManager* p;
   if (p->GetDoorStatus(doorId) != DAMSTATE_NOTPRESENT)
   ```
   Указатель `p` не инициализирован — разыменование мусора. Нужно
   `automobile->m_pDamageManager`.
2. `spite::active()` присваивает значения **локальным** переменным (`bool switc = switc1;`)
   вместо статических членов — функция ничего не делает.
3. `reload()`: `for (auto L : luastate) { luastate.pop_front(); }` — изменение контейнера
   во время итерации, неопределённое поведение (и состояния не закрываются `lua_close`).
4. `state::~state()` закомментирован — Lua-состояния никогда не освобождаются,
   плюс `lua_gc(LUA_GCSTOP)` отключает сборщик мусора → рост памяти при перезагрузках.

**Гонки и потокобезопасность:**
5. Все потоки создаются через `std::thread(...).detach()` без синхронизации; общий
   `list<lua_State*> luastate` и `listfile` правятся из разных потоков без mutex
   (mutex подключён, но не используется).
6. Вызовы игровых функций (plugin-sdk/SCM) из посторонних потоков небезопасны — игра
   однопоточная; возможны краши при обращении к сущностям во время стриминга.
   Надёжнее: накапливать задачи в очередь и исполнять их в `gameProcessEvent`.

**Совместимость:**
7. Жёсткие адреса `0x869630`, `0x9B5F08`, `0x974B2C`, `0x978748` — работает только
   на gta-vc.exe 1.0 (EN). На других версиях — краш. Лучше использовать
   `patch::get`/сигнатуры или поля классов SDK.
8. `std::experimental::filesystem` — устаревший API (C++14); в новом стандарте `<filesystem>`.
9. Кириллическое имя `setobjоcoordes` содержит русскую «о» внутри латиницы —
   невидимая ловушка при наборе имени функции в Lua.

## 7. Рекомендации

1. Исправить №1 и №2 сразу (реальные источники крашей).
2. Завести один глобальный `std::mutex` на `luastate`/`listfile`.
3. Перейти на модель «очередь команд»: Lua-поток только считает логику и кладёт
   задания, исполнение игровых команд — строго в игровом потоке.
4. Заменить сырые адреса на константы plugin-sdk (`MenuManager` уже есть в SDK —
   `CMenuManager` можно получать без хардкода).
5. Добавить `lua_close()` в деструктор `state` и вернуть включённым GC между
   перезагрузками.
6. Документировать API: список из `plugin.cpp` готов на 90% — вынести в Markdown
   (могу сгенерировать автоматически по аналогии со справочником plugin-sdk).

## 8. Состояние установки в игре

Проверено: `lualoader.asi` лежит в корне игры, папка `lualoader/` содержит рабочие
скрипты (`mod.lua` — таблица VK-кодов и хелперы, `test .lua`, `write_coordinate.lua`),
лог от 24.08.2026 подтверждает успешную загрузку всех трёх. Плагин рабочий.
...
по итогам обновления plugin-sdk и пересборки.*
