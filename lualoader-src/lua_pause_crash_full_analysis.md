...
## 14. ДОПОЛНЕНИЕ: КОНКРЕТНЫЕ АДРЕСА КРАША (из анализа 09:32 с gta-vc.exe, PE32 x86)

> **ВАЖНО: игра запущена как 32-битный `gta-vc.exe` (PE32, base 0x00400000), а НЕ
> reVC.exe (тот 64-битный, base 0x140000000).** Все адреса краша ниже привязаны к
> образу `gta-vc.exe` и system-области 32-битного процесса.

### 14.1 Архитектура (по факту исходника)
- 5 Lua-скриптов, каждый в отдельной **Lua VM** (`luaL_newstate`) + отдельный
  **`std::thread`** (`pump_owner`); т.е. каждый скрипт = свой thread + свой lua_State.
- `create_newthread()` из Lua плодит **дополнительную Lua VM + дополнительный
  C++ thread** (`luaL_newstate` + `std::thread(...).detach()`), с задержкой 301мс.
- Корутины: есть `L1` (вторая корутина через `lua_newthread(L)`), но `pump_owner`
  запускает `lua_resume(L1,...)` и `lua_resume(L,...)` **из своего** C++ thread.
- Игровые API (`Command<COMMAND_*>`, 504+ функций) дёргаются **напрямую из
  C++ worker-потоков** параллельно игровому потоку.
- Пауза по Ctrl = `scripts_disabled=true` + `pause_scripts()` (барьер по
  `active_pump_threads`) + (в прошлых версиях) запись в игровую память
  `ScriptSpace[OnAMissionFlag]=false` и `AddMessageJumpQ`.
- Остановка: yield-корутины через `lua_yield` из hook (была попытка), позже —
  «скрипт сам доезжает до своего `wait()` и паркуется». Teardown — асинхронный,
  отдельный поток, закрывает чужие lua_State.

### 14.2 Последний краш — точная находка (трейс 09:32, последние строки)
```
...
Ранее в той же сессии (стр. ≈8146): `VECTORED: code=0xC0000005 addr=0x7BCD72F0 thread=284`
и `code=0xE06D7363` («MS» C++ exception) на игровом потоке **сразу после** применения паузы.
...
1. **`0x004630EE` = внутри образа `gta-vc.exe`** (PE32, imagebase `0x400000`, .text),
   ...
   ```
   Т.е. **сама игра** в главном потоке разыменовала `edx+0x3AC`, где `edx` = результат
   предыдущего `call 0x451cb0` — это указатель на какой-то игровой объект (похоже,
   пед/авто), который стал **невалидным** (освобождён/перезаписан из worker-потока).
   Офсет `+0x3AC` — поле флагов/stay-was-attacked (перекликается с
   `set_ped_stay_when_attacked`!).

2. `0x7725EFDD`, `0x7BCD72F0` — **system-область** 32-битного процесса
   (ntdll/kernel32/winex11), массовые AV сразу на 3+ worker-потоках при падении
   игрового потока.

3. **Дата-пункт:** краш происходит **в КОДЕ ИГРЫ**, а не в `lualoader`. Значит
   `gta-vc.exe` доходит до строки чтения `[edx+0x3AC]` с испорченным `edx`. Это
   симптом **повреждения игровой памяти, вызванного НАМИ** — из worker-потоков
   параллельно с игровым потоком.

### 14.3 Причина (главный вывод)
...
- Lua-скрипты дергают `Command<COMMAND_*>`: `set_ped_stay_when_attacked`,
  `set_current_weapon_ped`, `is_ped_in_car`, `exitcar`, `create_spec_ped`,
  `mission_watch`, `setflagmission` — все пишут поля `CPed`/машины/флаги.
- **Игровой поток** каждый кадр читает те же структуры (AI, коллизии, рендер).
- Нет никакой блокировки: `active_pump_threads`/барьер защищают только «момент
  паузы», но не «вообще всю игру». Порча накапливается и детонирует в произвольной
  точке (часто — в момент паузы, т.к. в паузу скрипты паркуются на давно сломанных
  структурах).

Совпадает с выводом сервиса: главный риск — не пауза как таковая, а **500+
параллельных `Command<>` из чужих потоков**. Точка `0x4630EE` чтения
`[edx+0x3AC]` — прямая «дыра»: `set_ped_stay_when_attacked` пишет ровно эту зону
(stay-флаг).

### 14.4 Рекомендация сервиса (суть)
**Правильная схема потоков:**
1. **Один Lua-VM на скрипт** (`luaL_newstate`), доп. корутины — только
   `lua_newthread(L)` (общий registry/allocator), **НЕ** `luaL_newstate`.
2. **Один C++ thread-owner на каждый главный скрипт** (`pump_owner`).
3. **Ctrl:** только `scripts_disabled.store(true)`. Никакого `lua_close`/`lua_yield`
   из чужих потоков. Скрипт сам доезжает до своего `wait()`, внутри:
   `if (scripts_disabled.load()) return lua_yield(L,0);` → owner получает
   управление и **сам закрывает свой lua_State** (см. идея `owner_selfclose`).
4. **Не трогать `teardown_all`**, пока не найдена точка.
5. **Проверять код возврата `lua_resume()`** (LUA_ERRRUN/ERRMEM) — сейчас НЕ
   проверяется.
6. Не пытаться «всем скриптом» снять ET-флаги и синхронно остановить; дать
   скриптам **самим** завершиться через wait().

### 14.5 Не решено / открыто (на текущий момент)
1. Настоящая причина падения игрового потока — порча объекта игры (вероятно `CPed`,
   поле `+0x3AC`) из worker-потоков через `Command<>`/peds. **Недавние
   правильные адреса:** `0x4630EE` (игра), `0x7725EFDD`/`0x7BCD72F0`
   (system, worker-потоки).
2. Существует ли риск «повреждения до паузы»? Судя по адресу — ДА, повреждение
   идёт непрерывно во время работы миссии (стр. 8146, 8231 — все в момент миссии
   «Свинья»).
3. Проверить, что именно пишут 504 `Command<>` из worker-потоков, и где нет
   синхронизации.

### 14.6 Следующие шаги (план)
1. **Гипотеза подтверждается дизассемблером** — это главный прогресс. Зафиксировать
   и двигаться в сторону сериализации игровых API либо полной переработки.
2. Вариант А (минимальный хирургический): запретить worker-потокам вызывать
   `Command<>`, которые пишут педов/машины «на лету» (или gating всех `Command<>`
   глобальным мьютексом, который держит игровой поток на время игрового цикла).
...
5. Дождаться запуска моста LMArena (port 8000, `run_arena_bridge.sh`) для полного
   разбора трейса сервисом.

---

## 15. ФАЙЛЫ-АРТЕФАКТЫ (обновлено)

- Трейс: `.../old GTA Vice City/lualoader/trace.txt` (последние краш-строки:
  8041-8059, 8146, 8231-8240).
- Игра: `gta-vc.exe` (PE32 x86, base 0x400000; `reVC.exe` рядом — PE32+ x64, НЕ
  используется).
- Исходники: `.../old GTA Vice City/lualoader-src/` (`plugin.cpp`, `funcs.cpp`,
  `build.sh`).
- Мост: `~/Downloads/fast_work/LMArena_Bridge/` (порт 8000; сейчас выключен).
- Текущий билд: `../lualoader.asi`, sha256
  `aed0d44ce1e52cc787a4759c94a2228e3b015f6cdbc1f4b454ad8569a710ced8`
  (6567927 байт, 2026-09-22 09:31).
- Этот объединённый документ свёрстан из всех 8 исходных файлов:
  `lua_freeze_debug.md`, `lua_pause_analysis.md`, `lua_pause_analysis_fixed.md`,
  `lua_crash_analysis.md`, `lua_crash_analysis_fixed.md`,
  `lua_crash_final_analysis.md`, `lua_pause_crash_problem.md`,
  `lua_pause_crash_problem (копия).md`.
...
такого восстановления; их содержимое дублирует оригиналы. В настоящем документе
все содержимые части восстановлены и сведены воедино.
