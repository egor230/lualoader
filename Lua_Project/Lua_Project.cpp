...
int startscipt(string res, char* luafile, list<lua_State*>& luastate) {// запуска скрипта.
	...
	if (status == 0) {// если нет ошибки в файле.	
		...
		char* x = strdup(er1.c_str());		writelog(x);
...
public: Message() {
	...
	Events::gameProcessEvent += [] {//обработчик событий игры.
		...
		//float radar_x = patch::GetFloat(0x68FD2C);
		//float radar_y = patch::GetFloat(0x68FD34);

		//float radar_x1 = patch::GetFloat(0x68FD38);
		//float radar_y1 = patch::GetFloat(0x68FD3C);
		//if (steep == 0) {
		//	steep = 1;
		//	radar_x -= 23.0;
		//	radar_y -= 20.0;
		...
		//	int address = 0x869630 + 0x5B;
			...
			//patch::SetFloat(0x68FD2C, radar_x);
			//patch::SetFloat(0x68FD34, radar_y);

			/*patch::SetFloat(0x68FD38, radar_x1);
			patch::SetFloat(0x68FD3C, radar_y1);*/
			//		}
...
int funs(lua_State* L) {// список функций.
	...
	lua_register(L, "change_controller", change_controller); // 363 изменить тип управление Standrad = 0, Classic = 1.
	lua_register(L, "is_randomfindped", is_randomfindped); // 364 есть ли пед в радиусе.

	lua_register(L, "set_car_all_tires", set_car_all_tires); // 365 проколоть все шины.
	lua_register(L, "is_randomfindcar", is_randomfindcar); // 366 Есть ли случайное авто в радиусе.
	lua_register(L, "start_mission", start_mission); // 367 Запуск миссии.

	lua_register(L, "setcar_mass_and_speed", setcar_mass_and_speed); // 368 изменить массу и скорость авто.
	lua_register(L, "set_threat_for_ped", set_threat_for_ped); // 369 03F1 Тип Ped Угроза Ped в представлении поля флага.
	lua_register(L, "clear_threat_for_ped", clear_threat_for_ped); // 370 опкод 03F2 используется для убирания угрозы от выбранных пешеходов. 

	lua_register(L, "exitcar", exitcar); // 371 выйти из авто.
