...
int change_controller(lua_State* L); // изменить тип управление Standrad = 0, Classic = 1.
int is_randomfindped(lua_State* L); // есть ли пед в радиусе.
int set_car_all_tires(lua_State* L); // проколоть все шины.
int is_randomfindcar(lua_State* L); // Есть ли случайное авто в радиусе.


int setcar_mass_and_speed(lua_State* L); // изменить массу и скорость авто.

int set_threat_for_ped(lua_State* L); // 03F1 Тип Ped Угроза Ped в представлении поля флага.
int clear_threat_for_ped(lua_State* L); // опкод 03F2 используется для убирания угрозы от выбранных пешеходов. 
int start_mission(lua_State* L);// Запуск миссии.
...
int lockstatus(lua_State* L) {// статус двери авто.
	try {
		if (LUA_TLIGHTUSERDATA == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2)) {// указатель на авто. 
			...
			*/
			unsigned int status = lua_tointeger(L, 2);// статус двери авто.
			switch (status) {
			case 0: {car->m_eDoorLock = DOORLOCK_NOT_USED;
				break; }//	0	CARLOCK_NONE

			case 1: {car->m_eDoorLock = DOORLOCK_UNLOCKED;
				break; }// 1 CARLOCK_UNLOCKED разблокированная машина

			case 2: {car->m_eDoorLock = DOORLOCK_LOCKED;
				break; }// 2	CARLOCK_LOCKED	запертая машина - свойства включают двери не могут упасть, 
					//никто не может войти, но может выйти, любой может войти, если дверь оставлена ​​широко открытой или упала

			case 3: {car->m_eDoorLock = DOORLOCK_LOCKOUT_PLAYER_ONLY;
				break; }//	3	CARLOCK_LOCKOUT_PLAYER_ONLY	заблокирован для игрока, не заблокирован для NPC

			case 4: {car->m_eDoorLock = DOORLOCK_LOCKED_PLAYER_INSIDE;
				break; }//	4	CARLOCK_LOCKED_PLAYER_INSIDE заблокирован, игрок застрял в машине.

			case 5: {car->m_eDoorLock = DOORLOCK_COP_CAR;
				break; }//	5	CARLOCK_LOCKED_INITIALLY заблокирован, когда не в машине, разблокирован, когда в машине

			case 6: {car->m_eDoorLock = DOORLOCK_FORCE_SHUT_DOORS;
				break; }//	6	CARLOCK_FORCE_SHUT_DOORS анимация никогда не оставит дверь открытой, всегда закрывает дверь
					
			case 7: {car->m_eDoorLock =	DOORLOCK_SKIP_SHUT_DOORS;
				break; }//	7	CARLOCK_LOCKED_BUT_CAN_BE_DAMAGED заперт, но двери могут упасть
	...
	try {
		if (LUA_TLIGHTUSERDATA == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2)) {
			...
			for (auto car : CPools::ms_pVehiclePool) {
				if (car != v && DistanceBetweenPoints(car->GetPosition(), p->GetPosition()) < radius && car->m_fHealth > 10) {
					car->CanPedExitCar(true);
					if (CPed * p1 = car->m_pDriver) {
						if (p1 != NULL && p1 != p) {
							//lua_pushboolean(L, true); 
							lua_pushlightuserdata(L, p1);// отправить в стек и получить из стека можно
							return 1;
						}
					}
				}
			...
			for (auto ped : CPools::ms_pPedPool) {
				if (ped != p && DistanceBetweenPoints(ped->GetPosition(), p->GetPosition()) < radius && ped->m_fHealth > 10) {
					//lua_pushboolean(L, true); 
...
int is_randomfindped(lua_State* L) {// есть ли пед в радиусе.
	try {
		...
		else { throw "bad argument in function is_randomfindped"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
};
...
int randomfindcar(lua_State* L) {//Найти случайное авто в радиусе.
	try { if (LUA_TLIGHTUSERDATA == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2)) {
			...
			if (p->m_bInVehicle && p->m_pVehicle != NULL) {// в авто пед?
				CVehicle* v = p->m_pVehicle;	}// получить указатель на хенлд авто в котором сидит томии.
			for (auto car : CPools::ms_pVehiclePool) {
				if (car != v && car != v1 && DistanceBetweenPoints(car->GetPosition(), p->GetPosition()) < radius && car->m_fHealth > 50) {
...
int is_randomfindcar(lua_State* L) {// Есть ли случайное авто в радиусе.
	try {
		...
		else { throw "bad argument in function is_randomfindcar"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
};
int randomfindobj(lua_State* L) {// найти случайный объект в радиусе.
...
int create_spec_ped(lua_State* L) {// создать спец педа.
	try {
		if (LUA_TSTRING == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2) &&
			...
			std::thread(expectations2, idped).detach();
			mappeds.emplace(ped, L);// добавить map для педов.
...
int set_car_all_tires(lua_State* L) {// проколоть все шины.
	try {
		if (LUA_TLIGHTUSERDATA == lua_type(L, 1) ) {//строка.

			const void* p1 = lua_topointer(L, 1);

			CVehicle* car = findcarinpool(p1);//  получить указатель на авто.
			for (int wheel = 0; wheel < 4; wheel++) {// номер колеса.
				Command<COMMAND_BURST_CAR_TYRE>(CPools::GetVehicleRef(car), wheel);
			};
			return 0;
		}
		else { throw "bad argument in function set_car_all_tires"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
	return 0;
};
...
int get_ammo_weapon_ped(lua_State* L) {// получить кол-во патроны текущего оружие педа.
	...
	//		CWeapon weapon = ped->m_aWeapons[ped->GetWeaponSlot];// > m_nActiveWeaponSlot];
...
int expectations(int model, CVehicle* car) {
	...
	while (true) {
		this_thread::sleep_for(chrono::milliseconds(60));// задержка
		if (car->IsVisible()) {
	...
	try {
		if (LUA_TSTRING == lua_type(L, 1) || LUA_TNUMBER == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2)
			...
			const char* name_model = lua_tostring(L, 1);// модель авто.
			 ...
			 model = find_in_map(car_model_list, name_model);
			};
			float x = lua_tonumber(L, 2); float y = lua_tonumber(L, 3);
			float z = lua_tonumber(L, 4); CVector pos = { x, y, z };
			...
			car->m_eDoorLock = DOORLOCK_UNLOCKED;
...
int get_clip_current_weapon(lua_State* L) {// получить кол-во патронов в обойме текущего оружие.
	...
	//		CWeapon w = ped->m_aWeapons[ped->GetWeaponSlot];
...
int ped_search_threat(lua_State* L) {// пед ищет угрозу.		
	try {
		if (LUA_TLIGHTUSERDATA == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2)) {// указатель на педа.
			...
			/*
			Функция `COMMAND_SET_CHAR_THREAT_SEARCH` является одной из команд, доступных в скриптовом языке CLEO для игр серии Grand Theft Auto. Она используется для установки уровня угрозы, на который реагирует пед (пешеход) в игре.

Синтаксис команды:

```
COMMAND_SET_CHAR_THREAT_SEARCH charHandle threatLevel
```

где:
- `charHandle` - идентификатор педа, у которого нужно установить уровень угрозы.
- `threatLevel` - уровень угрозы, на который нужно установить педа. Может быть одним из следующих значений:
  - `0` - пед не будет реагировать на игрока или на других педов.
  - `1` - пед будет реагировать на игрока, если он находится в его поле зрения (угроза со стороны игрока).
  - `2` - пед будет реагировать на всех педов в игре, если они находятся в его поле зрения (угроза со стороны других педов).
  - `3` - пед будет реагировать на игрока и на всех педов в игре, если они находятся в его поле зрения (смешанная угроза).

Например, чтобы установить уровень угрозы педу с идентификатором 1 на уровень 2 (угроза со стороны других педов), можно использовать следующий код на языке CLEO:

```
0A8D: set_char_threat_search 1 2
```

Обратите внимание, что использование этой команды может повлечь за собой негативные последствия для игрового процесса или настроения других игроков. Используйте ее только в целях экспериментов или в локальной одиночной игре.
...
int start_mission(lua_State* L) {// Запуск миссии.
	try {
		if (LUA_TNUMBER == lua_type(L, 1)) {// значение число.
			int number_mission = lua_tointeger(L, 1);// 
			Command<COMMAND_LOAD_AND_LAUNCH_MISSION_INTERNAL>( number_mission);
			return 0;
		}
		else { throw "bad argument in function start_mission"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
	return 0;
};
...
int setcardimension(lua_State* L) { // получить размер авто.
	try {
		if (LUA_TLIGHTUSERDATA == lua_type(L, 1)) {// указатель на педа.
			const void* p = lua_topointer(L, 1);
			...
			//CVector pos = { x, y, z };// вектор координат.
			//pos =data->m_vDimensions;
	/*		car->m_pHandlingData->fMass=0;
			car->m_pHandlingData->fTractionMultiplier = 0;*/
			//car->m_pHandlingData->m_vecCentreOfMass
...
int change_controller(lua_State* L) {// изменить тип управление Standrad = 0, Classic = 1.
	try {
		if (LUA_TNUMBER == lua_type(L, 1) ) {// значение число.
			...
			CMenuManager& menu = *(CMenuManager*)0x869630;
			const char* swict = lua_tostring(L, 1);
			/*00869630 - FrontEndMenuManager (CMenuManager)
			+0x0A - [Байт] - m_bHudMode
			+0x18 - [4 байта] - m_PrefsBrightness
			+0x20 - [Байт] - m_bSubtitles
			+0x22 - [Байт] - m_bWidescreen
			+0x25 - [Байт] - m_bFrameLimiter
			+0x38 - [Байт] - m_bMenuVisible
			+0x39 - [Байт] - m_bDemoQuit
			+0x3C - [Байт] - m_bExtraSplash
			+0x4C - [4 байта] - m_nGameLanguage
			+0x5B - [Байт] - состояние контроллера (Standrad = 0, Classic = 1)*/
			int address = 0x869630 + 0x5B;
			//*(char*)
			patch::SetChar(address, 0);
			return 0;
		}// int
		else { throw "bad argument in function change_controller"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
	return 0;
};
...
//void ChangeVehicleParams()
int setcar_mass_and_speed(lua_State* L) { // изменить массу и скорость авто.
	try {
		if (LUA_TLIGHTUSERDATA == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2)) {// указатель на педа.
			const void* p = lua_topointer(L, 1);
			float mass= lua_tointeger(L, 2);// масса авто.
			CVehicle* car = findcarinpool(p);// получить указатель на авто.
			//CVehicle* car = FindPlayerVehicle();

			// Получить указатель на структуру handlingData
			car->m_pHandlingData->fMass = 3000.0f;
			car->m_fMass = 3000.0f;
			//tHandlingData* handling = car->m_pHandlingData;
		
				 //car->m_pHandlingData->fMass = 4.5f;
	/*			handling->fMass = 30.0f;
				car->m_pHandlingData = handling;*/
			//float f = car->m_fVelocityFrequency;
			//writelog1(f);
			//car->m_fMass = 3000;
			//car->m_fVelocityFrequency= 0.5;
			//CModelInfo::AddVehicleModel(car->m_nModelIndex);
			//	}
			//}
			//lua_pushnumber(L, f);
			return 0;
		}
		else { throw "bad argument in function setcar_mass_and_speed"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
	return 0;
};

int set_threat_for_ped(lua_State* L) {// 03F1 Тип Ped Угроза Ped в представлении поля флага(см.Ниже)
	//	Этот код операции устанавливает враждебность типа ped к другим типам ped
	//и будет атаковать тип ped угрозы при первом же появлении.
	try {
		if (LUA_TNUMBER == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2) ) {// значение число.
				int value = lua_tointeger(L, 1);// модель педа.
				int type = lua_tointeger(L, 2);// тип педа.
		
				Command<COMMAND_SET_THREAT_FOR_PED_TYPE>(value, type);
			return 0;
		}// int
		else { throw "bad argument in function set_threat_for_ped"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
	return 0;
	/*
	...
	1  player2  2 
	player3 3  player4  4  
	civmale 5  civfemale 6
	cop    7  gang1 8 
	gang2 9  gang3  10  
	gang4 11  gang5 12 
	gang6 13  gang7 14 
	gang8 15  gang9 16
	emergency 17  fireman 18  
	criminal 19  special
 Gang0  Cubans  
Gang1  Haitians  
...
Отметить	Бинарный	Перечисление
GTA III	Вайс Сити
1	00 0000 0000 0000 0000 0000 0001	THREAT_PLAYER1 (УГРОЗА)
2	00 0000 0000 0000 0000 0000 0010	THREAT_PLAYER2 (УГРОЗА)
4	00 0000 0000 0000 0000 0000 0100	THREAT_PLAYER3 (УГРОЗА)
8	00 0000 0000 0000 0000 0000 1000	THREAT_PLAYER4 (УГРОЗА)
16	00 0000 0000 0000 0000 0001 0000	УГРОЗА ДЛЯ ЖИВЫХ
32	00 0000 0000 0000 0000 0010 0000	УГРОЗА ДЛЯ ГРАЖДАНСКОЙ ЖЕНЩИНЫ
64	00 0000 0000 0000 0000 0100 0000	THREAT_COP (УГРОЗА)
128	00 0000 0000 0000 0000 1000 0000	THREAT_GANG_MAFIA (УГРОЗА)	THREAT_GANG_CUBAN (УГРОЗА)
256	00 0000 0000 0000 0001 0000 0000	THREAT_GANG_TRIAD (ТРИАДА УГРОЗ)	THREAT_GANG_HAITIAN
512	00 0000 0000 0000 0010 0000 0000	THREAT_GANG_DIABLO	THREAT_GANG_STREET
1024	00 0000 0000 0000 0100 0000 0000	THREAT_GANG_YAKUZA	THREAT_GANG_DIAZ (УГРОЗА)
2048	00 0000 0000 0000 1000 0000 0000	THREAT_GANG_YARDIE (УГРОЗА)	THREAT_GANG_SECURITY
4096	00 0000 0000 0001 0000 0000 0000	THREAT_GANG_COLOMBIAN	THREAT_GANG_BIKER (УГРОЗА)
8192	00 0000 0000 0010 0000 0000 0000	THREAT_GANG_HOOD (УГРОЗА ОПАСНОСТИ)	THREAT_GANG_PLAYER (УГРОЗА)
16384	00 0000 0000 0100 0000 0000 0000		THREAT_GANG_GOLFER (УГРОЗА ДЛЯ ИГРЫ в ГОЛЬФ)
32768	00 0000 0000 1000 0000 0000 0000		УГРОЗА_GANG9
65536	00 0000 0001 0000 0000 0000 0000	УГРОЗА ВОЗНИКНОВЕНИЯ
131072	00 0000 0010 0000 0000 0000 0000	РАСПРОСТРАНЕНИЕ УГРОЗЫ
262144	00 0000 0100 0000 0000 0000 0000	THREAT_CRIMINAL (УГРОЗА ПРЕСТУПНИКА)
524288	00 0000 1000 0000 0000 0000 0000	ОСОБАЯ УГРОЗА
1048576	00 0001 0000 0000 0000 0000 0000	THREAT_GUN
2097152	00 0010 0000 0000 0000 0000 0000	THREAT_COP_CAR - АВТОМОБИЛЬ с УГРОЗОЙ
4194304	00 0100 0000 0000 0000 0000 0000	THREAT_FAST_CAR - БЫСТРЫЙ АВТОМОБИЛЬ
8388608	00 1000 0000 0000 0000 0000 0000	УГРОЗА ВЗРЫВА
16777216	01 0000 0000 0000 0000 0000 0000	ЧЕЛОВЕК-УГРОЗА ПОЖАРА
33554432	10 0000 0000 0000 0000 0000 0000	THREAT_DEADPEDS - СМЕРТЕЛЬНАЯ УГРОЗА
Пример
...
int clear_threat_for_ped(lua_State* L) {// опкод 03F2 используется для убирания угрозы от выбранных пешеходов. 
	//Он позволяет уменьшить уровень угрозы выбранного пешехода до нуля, что означает, что этот пешеход больше 
	//не будет нападать на игрока в случайной встрече.
	try {
		if (LUA_TNUMBER == lua_type(L, 1) && LUA_TNUMBER == lua_type(L, 2)) {// значение число.
			int value = lua_tointeger(L, 1);// модель педа.
			int type = lua_tointeger(L, 2);// тип педа.

			Command<COMMAND_CLEAR_THREAT_FOR_PED_TYPE>(value, type);
			return 0;
		}// int
		else { throw "bad argument in function clear_threat_for_ped"; }
	}
	catch (const char* x) { writelog(x); }// записать ошибку в файл.
	return 0;
};
